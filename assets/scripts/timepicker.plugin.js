;(function ( $ ) {
	...
	// TIMEPICKER DEFAULT DROPDOWN-STYLE
	// ---------------------------------------------------------------------------
	$.als.plugins.timePicker = function (timepicker, config) {

		this.utiliites = $.als.utilities;

		this.timepicker = timepicker;

		this.timepickerID = timepicker.attr('id');

		this.numTimepickers = $('.timepicker').length;
	...
	$.als.plugins.timePicker.prototype = {
		createTimepickerDropdown: function () {

			// Dropdown template
			var labelCount,
				timePickerHTML,
				timePickerWrapper   = this.templates.timePickerWrapper,
				timePickerSeparator = this.templates.separatorText,
				dropdownOption      = this.templates.dropdownOption,
				timeZoneLabel       = this.templates.timeZoneLabel,
				dropdownTemplate    = this.templates.dropdown,
				self                = this;

			// replace template placeholders
			timePickerHTML = this.utiliites.renderTemplate(dropdownTemplate, {
				'@modifierClass': this.config.modifierClass || '',
				'@options': this.optionsArray.join('')
			});

			// Append duplicate dropdown if stepped timepicker
			if (this.config.step) {
				timePickerHTML += this.utiliites.renderTemplate(timePickerSeparator, {
					'@separatorText': this.config.separatorText || 'TO'
				}) + timePickerHTML;
			}

			// Append time zone
			if (this.config.timezone) {
				timePickerHTML += this.utiliites.renderTemplate(timeZoneLabel, {
					'@timeZoneLabel': this.config.timezone || ''
				});
			}

			// Prepend dropdown label
			timePickerHTML = self.utiliites.renderTemplate(timePickerHTML, {
				'@label': self.utiliites.renderTemplate(dropdownOption, {
					'@optionText': 'Select Time'
				})
			});

			// Replace timepicker directive with timepicker markup
			this.timepicker.replaceWith(
				this.utiliites.renderTemplate(timePickerWrapper, {
					'@id': this.timepickerID,
					'@timePickerHTML': timePickerHTML
				})
			);

			this.$timepicker = $('#' + this.timepickerID);
		...
		selectDefaultTimes: function () {

			if (this.config.selectedStartTime) {
				$('.select__menu:eq(0)', this.$timepicker).val(this.config.selectedStartTime);
			}
			if (this.config.selectedEndTime) {
				$('.select__menu:eq(1)', this.$timepicker).val(this.config.selectedEndTime);
			}
		...
		setBlockedOutTimes: function () {
			...
			var exclusions = this.config.blockOutTimes,
					self       = this;

			exclusions.forEach(function (exclusion) {
				$('option[value="' + exclusion + '"]', self.$timepicker).attr('disabled', true);
			});
		...
		setDropdownOptions: function () {
			...
			var value, optionStr, originalOptionsStr, newOptionStr, i;

			var startIndex = this.config.startTime.match('am')
								? parseInt(this.config.startTime)
								: 12 + parseInt(this.config.startTime);

			var endIndex = this.config.endTime.match('am')
								? parseInt(this.config.endTime)
								: 12 + parseInt(this.config.endTime);

			startIndex = this.config.startTime === '12:00am' ? 0 : startIndex;

			this.optionsArray = [];

			for (i = startIndex; i <= endIndex; i += 1) {

				// Create option string based on hourly time intervals
				if (i !== 12 && i !== 0) {
					value = (i > 12) ? (i - 12) + ':00pm' : i + ':00am';
					optionStr = '<option value="' + value + '">' + value + '</option>';
				}
				else if (i === 0) {
					optionStr ='<option value="12:00am">12:00am</option>';
				}
				else {
					optionStr ='<option value="12:00pm">12:00pm</option>';
				}

				// If 30 minute intervals
				if (this.config.interval === 30 && i !== endIndex) {
					optionStr +=  optionStr.replace(/\:00/g, ':30');
				}

				// If 15 minute intervals
				if (this.config.interval === 15 && i !== endIndex) {

					originalOptionsStr = optionStr;
					newOptionStr = '';

					[':15',':30',':45'].forEach(function (interval) {
						newOptionStr += originalOptionsStr.replace(/\:00/g, interval);
					});

					optionStr += newOptionStr;
				}

				this.optionsArray.push(optionStr);
			}

			return this;
		},
		templates: {
			dropdown:          '<div class="select @modifierClass"><select class="select__menu">@label @options</select><div class="select__icon"></div></div>',
			separatorText:     ' <span class="timepicker__label txt--bold">@separatorText</span> ',
			timeZoneLabel:     ' <span class="timepicker__label">@timeZoneLabel</span>',
			dropdownOption:    '<option>@optionText</option>',
			timePickerWrapper: '<div id="@id" class="timepicker">@timePickerHTML</div>'
		}
	};
	...
	$.fn.timepicker = function (config) {
		...
		return this.each(function () {
			if (typeof config.type !== 'undefined' && config.type === 'scheduler') {
				timepicker = new $.als.plugins.timePickerScheduler($(this), config);
			}
			else {
				timepicker = new $.als.plugins.timePicker($(this), config);
			}
		});
	};
